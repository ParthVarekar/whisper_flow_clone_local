...
class Qwen3AsrBackend(TranscriptionBackend):
    ...
    def _build_cmd(self, audio_path: str, *, initial_prompt: str = "") -> list[str]:
        ...
        if is_crispasr:
            ...
            if initial_prompt:
                # Explicit English-only instruction to prevent Hindi/other language
                # leakage. Qwen3-ASR uses the system prompt as background knowledge.
                context = f"Transcribe in English only. Technical terms: {initial_prompt}"
                cmd.extend(["--prompt", context])
            else:
                # Even without vocabulary, force English to prevent Hindi leakage
                cmd.extend(["--prompt", "Transcribe in English only."])
